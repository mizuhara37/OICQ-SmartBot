...
logger.info('=================================');
logger.info('          OICQ-SmartBot');
logger.info('       作者：SinanGentoo  改编：Mizuhara37');
logger.info('=================================');
...
function loadComponents () {
    ...
    for (const componentId in config.code) {
        const name = config.code[componentId];
        try {
            const Component = require(`./code/${name}`);
            components[name] = new Component(client, guild);
            if (components[name].activate) components[name].activate();
            logger.info(`组件 ${name} 已被激活!`);
        } catch (e) {
